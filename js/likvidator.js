function Likvidator() {
    const currentParagraph = $('#currentL');
    oDbUloge.orderByChild('naziv').equalTo('likvidator').limitToLast(1).once('value', (rolesSnapshot) => {
      ...
      if (rolesData) {
        ...
        oDbKorisnici.child(korisnikId).once('value', (userSnapshot) => {
            ...
            if (user.status === 4) {
              ...
              currentParagraph.text(`Trenutni likvidator: ${Name}`);
            } else {
              currentParagraph.text('Podaci o likvidatoru nisu dostupni.');
            }
        });
      } 
      else {
        currentParagraph.text('Trenutno nema likvidatora.');
      }
    });
  }
...
function Old() {
  return new Promise((resolve, reject) => {
    oDbUloge.orderByChild('naziv').equalTo('likvidator').once('value')
      .then((snapshot) => {
        ...
        if (Data) {
          ...
          reject('Nema trenutnog likvidatora.');
        }
      })
      .catch((error) => reject(error));
  });
...
function New(selectedUserId) {
  ...
  return oDbUloge.update(oZapis)
    .then(() => {
      return oDbKorisnici.child(selectedUserId).update({ status: 4 });
    });
}
...
function displayTable() {
    const TableBody = $('#tableChooseL tbody');
    oDbUloge.orderByChild('naziv').equalTo('likvidator').once('value', (rolesSnapshot) => {
      const rolesData = rolesSnapshot.val();
      if (rolesData) {
        ...
        const noRow = '<tr><td colspan="3">Trenutno nema likvidatora.</td></tr>';
...
function liveSearch() {
  ...
  $('#tableChooseL tbody tr').filter(function() {
    $(this).toggle(
                  ...
                  oDbUloge.orderByChild('naziv').equalTo('likvidator').once('value')
                    ...
                    if (Data) {
                      ...
                      oDbUloge.child(lastId).update({ datum_zavrsetka: Danas() }).then(() => {
                        alert('Član uklonjen s uloge likvidator');
                        location.reload();
                      });
                    } 
                  });
