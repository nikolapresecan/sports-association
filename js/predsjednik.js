function President() {
    const currentParagraph = $('#currentP');
    oDbUloge.orderByChild('naziv').equalTo('predsjednik').limitToLast(1).once('value', (rolesSnapshot) => {
      ...
      if (rolesData) {
        ...
        oDbKorisnici.child(korisnikId).once('value', (userSnapshot) => {
            ...
            if (user.status === 1) {
              ...
              currentParagraph.text(`Trenutni predsjednik: ${Name}`);
            } else {
              currentParagraph.text('Podaci o predsjedniku nisu dostupni.');
            }
        });
      } 
      else {
        currentParagraph.text('Trenutno nema predsjednika.');
      }
    });
  }
...
function Old() {
  return new Promise((resolve, reject) => {
    oDbUloge.orderByChild('naziv').equalTo('predsjednik').once('value')
      .then((snapshot) => {
        ...
        if (Data) {
          ...
          reject('Nema trenutnog predsjednika.');
        }
      })
      .catch((error) => reject(error));
  });
...
function New(selectedUserId) {
  ...
  return oDbUloge.update(oZapis)
    .then(() => {
      return oDbKorisnici.child(selectedUserId).update({ status: 1 });
    });
}
...
function displayTable() {
    const TableBody = $('#tableChooseP tbody');
    oDbUloge.orderByChild('naziv').equalTo('predsjednik').once('value', (rolesSnapshot) => {
      const rolesData = rolesSnapshot.val();
      if (rolesData) {
        ...
        const noRow = '<tr><td colspan="3">Trenutno nema predsjednika.</td></tr>';
...
function liveSearch() {
  ...
  $('#tableChooseP tbody tr').filter(function() {
    $(this).toggle(
                  ...
                  oDbUloge.orderByChild('naziv').equalTo('predsjednik').once('value')
                    ...
                    if (Data) {
                      ...
                      oDbUloge.child(lastId).update({ datum_zavrsetka: Danas() }).then(() => {
                        alert('Član uklonjen s uloge predsjednik');
                        location.reload();
                      });
                    } 
                  });
